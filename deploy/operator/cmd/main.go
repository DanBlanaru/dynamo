...
func registerControllers(
	...
	if err := controller.SetupDynamoComponentDeployment(mgr, setupOptions); err != nil {
		return err
	}
	if err := controller.SetupDynamoGraphDeployment(mgr, setupOptions); err != nil {
		return err
	}
	if err := controller.SetupDynamoGraphDeploymentScalingAdapter(mgr, setupOptions); err != nil {
		return err
	}
	if err := controller.SetupDynamoGraphDeploymentRequest(mgr, setupOptions); err != nil {
		return err
	}
	if err := controller.SetupDynamoModel(mgr, setupOptions); err != nil {
		return err
	}
	...
	if runtimeConfig.GroveEnabled {
		if err := controller.SetupFailoverCascade(mgr, setupOptions); err != nil {
			return err
		}
	}
