...
The OpenAI-compatible completion routes provide the current token-in/token-out interface. The
experimental `POST /inference/v1/generate` route validates its request shape but currently returns
HTTP 501; do not use it for rollouts.
