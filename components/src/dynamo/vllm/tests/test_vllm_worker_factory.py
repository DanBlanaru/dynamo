...
from dynamo.vllm.main import _base_model_lora_capacity
...
@pytest.mark.parametrize(
    ...
    [
        (True, ModelType.Prefill, 3),
        (True, ModelType.Chat, 3),
        (True, ModelType.Embedding, None),
        (False, ModelType.Prefill, None),
    ],
)
def test_base_model_lora_capacity(
    enable_lora: bool, model_type: ModelType, expected: int | None
) -> None:
    config = Mock(
        engine_args=SimpleNamespace(enable_lora=enable_lora, max_loras=3)
    )

    assert _base_model_lora_capacity(config, model_type) == expected
